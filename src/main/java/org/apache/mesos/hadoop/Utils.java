...
public class Utils {
	...
	public static ContainerInfo buildContainerInfo(Configuration conf) {
		...
		// String[] containerOptions = conf
		//			.getStrings("mapred.mesos.container.options");
		...
		if (containerImage != null) {
			...
			dockerInfoBuilder.setImage(containerImage);
			DockerInfo dockerInfo = dockerInfoBuilder.build();
...
//		}
	...
	public static ContainerInfo buildDockerContainerInfo(Configuration conf) {
		...
		dockerInfoBuilder.setPrivileged(conf.getBoolean(
				"mapred.mesos.docker.privileged", false));
		dockerInfoBuilder.setForcePullImage(conf.getBoolean(
				"mapred.mesos.docker.force_pull_image", false));
		...
		if (params != null && params.length > 0) {
			...
			if ((params.length % 2) != 0) {
				throw new IllegalArgumentException(
						"The number of parameters should be even, k/v pairs");
			}
		...
		if (volumes != null && volumes.length > 0) {
			for (int i = 0; i < volumes.length; i++) {
				...
				if (parts.length <= 1 || parts.length > 3) {
					throw new IllegalArgumentException(
							"Invalid volume configuration (host_path:container_path:[rw|ro])");
				}
				...
				if (parts.length == 2) {
					containerInfoBuilder.addVolumes(Volume.newBuilder()
							.setContainerPath(parts[0]).setMode(mode).build());
				} else {
					containerInfoBuilder.addVolumes(Volume.newBuilder()
							.setHostPath(parts[0]).setContainerPath(parts[1])
							.setMode(mode).build());
				}
			}
		}
